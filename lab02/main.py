...
from lab02.algorithms.greedy_cycle import GreedyCycleAlgorithm
...
if __name__ == '__main__':
    for problem_file in ("kroA100.tsp", "kroB100.tsp"):
        ...
        best_solution_greedy[1].plot(problem.points, f"GreedyLocal vert: best result for {problem_file}", show=True)

        print(f"algorithm: Random\n\tbest: {min(results)}\n\tworst: {max(results)}\n\tavg: {sum(results)/len(results)}")
        print(f"algorithm: GreedyLocal edge\n\tbest: {min(res[0])}\n\tworst: {max(res[0])}\n\tavg: {sum(res[0])/len(res[0])}")
        print(f"algorithm: GreedyLocal vert\n\tbest: {min(res[1])}\n\tworst: {max(res[1])}\n\tavg: {sum(res[1])/len(res[1])}")
