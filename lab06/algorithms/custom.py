...
class CustomRegretAlgorithm(Algorithm):
    def run(self, start1=None):
        ...
        start2 = self.data[start1].argmax()
        # max_dist = self.data[start1, start2]
        ...
        for i in range(1, nodes_n // 2 - 1):
            for ic, cycle in enumerate(cycles):
                ...
                for n, n_visited in enumerate(visited):
                    ...
                    regrets[n] = insert_options[0, 0], \
                                 w1 * (insert_options[1, 1] - insert_options[0, 1]) - w2 * insert_options[
                                     0, 1] + 999_999

                # best node, best spot (for best node), regret of it
                non, rien_de_rien, je_ne_regrette_rien = -np.inf, -np.inf, -np.inf
